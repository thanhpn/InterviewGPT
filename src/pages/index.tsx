...
const Home: NextPage = () => {
    ...
    const [name, setName] = React.useState<string>("InterviewGPT");
    const [goalInput, setGoalInput] = React.useState<string>("");
    const [intervieweeAnswer, setIntervieweeAnswer] = React.useState<string>("");
    ...
    const handleSubmitAnswer = () => {
        console.log("handleSubmitAnswer", intervieweeAnswer)
        if (agent) {
            agent.submitAnswer(intervieweeAnswer).then(console.log).catch(console.error);
        }
    };

    const handleCreateAnswer = () => {
        console.log("handleCreateAnswer")
        if (agent) {
            agent.createAnswer().then(console.log).catch(console.error);
        }
    };
    ...
    const handleNextQuestion = () => {
        ...
        if (agent) {
            agent.nextQuestion().then(console.log).catch(console.error);
        }
    };

    const proTitle = (
        <>
            InterviewGPT<span className="ml-1 text-amber-500/90">Pro</span>
        </>
    );
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="mt-1 text-center font-mono text-[0.7em] font-bold text-white">
                                <p>
                                    {t(
                                        "Assemble, configure, and deploy autonomous AI Interview in your browser."
                                    )}
                                </p>
                            </div>
                        </div>
                        ...
                        <Expand className="flex w-full flex-row">
                            <ChatWindow
                                className="sm:mt-4"
                                messages={messages}
                                title={session?.user.subscriptionId ? proTitle : "InterviewGPT"}
                                showDonation={
                            ...
                            <Expand delay={1.3}>
                                <Input
                                    left={
                                        <>
                                            <FaStar />
                                            <span className="ml-2">{t("Topic")}</span>
                                        </>
                                    }
                                    disabled={agent != null}
                                    value={goalInput}
                                    ...
                                    onKeyDown={(e) => handleKeyPress(e)}
                                    placeholder={`${t("The topic of this interview meeting")}`}
                                    type="textarea"
                                />
                            </Expand>
                        ...
                        <Expand delay={1.4} className="flex gap-2">
                            <Button
                                ...
                                className="sm:mt-10"
                                enabledClassName={"bg-green-600 hover:bg-blue-400"}
                            >
                                {shouldAgentStop ? (
                                    <>
                                        <VscLoading className="animate-spin" size={20} />
                            ...
                            <Button
                                ...
                                className="sm:mt-10"
                                enabledClassName={"bg-yellow-600 hover:bg-blue-400"}
                            >
                                {shouldAgentStop ? (
                                    <>
                                        <VscLoading className="animate-spin" size={20} />
