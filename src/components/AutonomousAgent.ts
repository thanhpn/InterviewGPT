...
class AutonomousAgent {
    ...
    constructor(
        ...
        this.currentQuestion = {
            taskId: v1().toString(),
            value: 'Please wait, Iam preparing the first question...',
    ...
    async run() {
        ...
        try {
            ...
            if (this.tasks.length > 0) {
                this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: 'Are you ready? We will start interview now, please focus on the question' });
            } else {
                this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: 'No question created, please start interview again' });
                this.shutdown();
                return;
            }
        } catch (e) {
    ...
    async loop() {
        ...
        await new Promise((r) => setTimeout(r, TIMEOUT_LONG));
        // this.sendThinkingMessage();
        this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: `I'm waiting for your answer...` });
    }
    ...
    async submitAnswer(answer: string) {
        ...
        this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: `Your answer: ${answer}` });
        this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: 'Checking your answer...' });
        // check answer
        this.sendThinkingMessage();
        ...
        this.sendMessage({ type: MESSAGE_TYPE_SYSTEM, value: result });
        await new Promise((r) => setTimeout(r, TIMEOUT_LONG));
        this.sendThinkingMessage();
    ...
    sendLoopMessage() {
        this.sendMessage({
            ...
            value: !!this.modelSettings.customApiKey
                ? `This interview has maxed out on loops. To save your wallet, this agent is shutting down. You can configure the number of loops in the advanced settings.`
    ...
    sendManualShutdownMessage() {
        this.sendMessage({
            type: MESSAGE_TYPE_SYSTEM,
            value: `The interview has been manually stop.`,
        });
    }
